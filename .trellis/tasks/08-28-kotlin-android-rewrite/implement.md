...
## Homepage Carousel Layout Follow-up

- [x] 首页横向列表在手机、平板与桌面断点按容器宽度精确显示 2/3/4/5 张完整卡片。
- [x] 左右按钮按一个完整视口翻页，滚动与 resize 后仍正确更新可用状态。
- [x] 登录与游客“继续观看”复用同一卡片尺寸规则，其他首页区块不再出现半张卡片。
- [x] 增加布局契约测试，并在 320、375、768、1024、1440 宽度做截图与溢出检查。
