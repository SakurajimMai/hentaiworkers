-- Additive crawler control-plane schema (MariaDB 11.4+)
-- Does NOT alter or drop existing catalog tables (animes, tags, anime_tags, categories, users).
...
CREATE TABLE IF NOT EXISTS `crawler_profile_versions` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `crawler_profile_versions_profile_version_uidx` (`profile_id`, `version`),
  KEY `crawler_profile_versions_profile_id_idx` (`profile_id`),
  CONSTRAINT `crawler_profile_versions_profile_fk`
    FOREIGN KEY (`profile_id`) REFERENCES `crawler_profiles` (`id`),
  CONSTRAINT `crawler_profile_versions_config_json_valid`
...
CREATE TABLE IF NOT EXISTS `secret_versions` (
  ...
  `created_by_user_id` BIGINT UNSIGNED NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `secret_versions_secret_version_uidx` (`secret_id`, `version`),
...
CREATE TABLE IF NOT EXISTS `crawler_schedules` (
  ...
  CONSTRAINT `crawler_schedules_profile_fk`
    FOREIGN KEY (`profile_id`) REFERENCES `crawler_profiles` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `crawler_workers` (
  ...
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  CONSTRAINT `crawler_workers_capabilities_json_valid`
...
CREATE TABLE IF NOT EXISTS `crawler_jobs` (
  ...
  CONSTRAINT `crawler_jobs_schedule_fk`
    FOREIGN KEY (`schedule_id`) REFERENCES `crawler_schedules` (`id`),
  CONSTRAINT `crawler_jobs_config_snapshot_json_valid`
    CHECK (JSON_VALID(`config_snapshot_json`)),
  CONSTRAINT `crawler_jobs_progress_json_valid`
...
CREATE TABLE IF NOT EXISTS `crawler_job_attempts` (
  ...
  CONSTRAINT `crawler_job_attempts_job_fk`
    FOREIGN KEY (`job_id`) REFERENCES `crawler_jobs` (`id`),
  CONSTRAINT `crawler_job_attempts_worker_fk`
    FOREIGN KEY (`worker_id`) REFERENCES `crawler_workers` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `crawler_job_items` (
  ...
  CONSTRAINT `crawler_job_items_job_fk`
    FOREIGN KEY (`job_id`) REFERENCES `crawler_jobs` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `crawler_job_events` (
  ...
  CONSTRAINT `crawler_job_events_job_fk`
    FOREIGN KEY (`job_id`) REFERENCES `crawler_jobs` (`id`),
  CONSTRAINT `crawler_job_events_payload_json_valid`
...
CREATE TABLE IF NOT EXISTS `crawler_operation_receipts` (
  ...
  CONSTRAINT `crawler_operation_receipts_response_json_valid`
...
CREATE TABLE IF NOT EXISTS `worker_credentials` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `worker_credentials_token_hash_uidx` (`token_hash`),
  KEY `worker_credentials_worker_id_idx` (`worker_id`),
  CONSTRAINT `worker_credentials_worker_fk`
    FOREIGN KEY (`worker_id`) REFERENCES `crawler_workers` (`id`),
  CONSTRAINT `worker_credentials_scope_json_valid`
    CHECK (JSON_VALID(`scope_json`))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `audit_logs` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `actor_type` ENUM('admin', 'worker', 'system') NOT NULL,
  ...
  CONSTRAINT `audit_logs_payload_json_valid`
    CHECK (`payload_json` IS NULL OR JSON_VALID(`payload_json`))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `media_assets` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `anime_id` BIGINT UNSIGNED NOT NULL,
  `kind` ENUM('video', 'cover', 'fanart') NOT NULL,
  `storage_driver` ENUM('s3', 'sftp', 'external') NOT NULL,
  ...
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `media_assets_anime_id_idx` (`anime_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
