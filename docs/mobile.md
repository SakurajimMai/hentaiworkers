...
或用低清图替代最终原图。预览可用时先显示预览，同时立即加载原图。阅读窗口沿当前方向准备
后续两页的预览和更远四页的磁盘文件，最多同时运行两项预取，首屏最多保留 300 毫秒的优先加载
时间。原图的 Coil 过渡位图限制在 `960x2560` 以内，静态图片的清晰内容仍由 Telephoto 从完整
磁盘文件按需分块显示；调整阅读比例不会重新下载原图。已知图片比例随页面状态保留，回滚翻页
时无需先恢复成统一占位高度。离开或切换阅读入口会取消尚未完成的后续页预取，避免无效下载；
广告也要等首张原图显示后才加载。
