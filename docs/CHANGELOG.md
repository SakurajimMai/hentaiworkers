...
- 新增后台开关「系统 -> 漫画阅读 -> 阅读页直连图床」（默认关闭）。开启后，网页阅读器跳过页图调度，
  每页直接渲染指向原图地址的 `<img loading="lazy">`；章节接口 `/api/mangas/{id}/chapters/{number}`
  新增 `directImages` 字段，新版 APK 据此直接请求原图地址、不改写到 `/cdn-img`，并停用目标页预热、
  翻页预取和下一话预热。章节里的图片地址在两种模式下完全相同；旧版 APK 忽略该字段，继续走代理。
  开启前需确认用户网络能直接访问图床域名。
...
## 2026-09-22 — 邮箱验证通过后才创建账号

- 发送验证码仅保存待验证申请和密码哈希，验证成功后在同一数据库事务中创建用户并消费验证码。
- 发送失败、验证码错误、过期和重复提交均不能创建账号；验证完成后需登录。
- 新申请的 120 秒发送间隔由数据库统一控制，部署前执行 `0021-pending-registrations.sql`。
- 保留旧版未激活用户的验证路径，不自动删除既有账号。
