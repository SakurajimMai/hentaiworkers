...
主站消费目录中配置的媒体 URL，不负责通用视频代理。`/cdn-img/<host>/**` 只代理与 `SITE_URL`
同一域名下的图片主机（如 `image1.example.com`、`image2.example.com`，域名由 `SITE_URL` 推导，
无需配置），拒绝站点自身与域外主机；不能把它描述成任意图片或视频代理。没有主机段的旧路径
（Build 111 及更早的 APK）按最新目录封面出现过的主机依次尝试。
