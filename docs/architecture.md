...
AnimeStream 是一个 Next.js 模块化单体。仓库、Node.js 进程和生产 Compose 都只包含主站：公开站点、管理后台、公开 API 和 MySQL/MariaDB 数据访问。

数据抓取、媒体下载、对象存储上传及其调度不在本系统边界内。主站不提供对应的内部控制 API，也不读取相关环境变量或本机共享目录。
