...
class UnifiedCrawler:
        ...
        return {
            'crawl': {
                ...
                'no_subtitle_markers': [
                    '無字幕', '无字幕', '無中字', '无中字',
                    '無字', '无字', '生肉',
                    '[無字]', '[无字]', '[無字幕]', '[无字幕]', '[無中字]', '[无中字]',
                ],
                'skip_keywords': [
                    '中字後補', '简中补字', '中文字幕後補', '後補字幕', '补字', '後補',
                ],
                'subtitle_positive_markers': [
                    '[中文字幕]', '中文字幕', '中文配音', '中字', '简中', '繁中',
                    'Chinese Sub', 'CHS',
                ],
            ...
            'database': {
                # Prefer env: MYSQL_HOST / MYSQL_USER / MYSQL_PASSWORD / MYSQL_DATABASE
                'host': '127.0.0.1',
                'port': 3306,
                'user': 'root',
                'password': '',
    ...
    def setup_database(self):
        """设置数据库连接（仅 MySQL/MariaDB，支持环境变量覆盖）"""
        import os
        db_config = self.config.get('database', {}) or {}
        ...
        if ':' in str(host):
            host_ip, port_s = str(host).rsplit(':', 1)
            port = int(port_s)
        else:
            host_ip = str(host)
            port = int(
                os.environ.get('MYSQL_PORT')
                or db_config.get('port')
                or 3306
            )
        ...
        self.db_config = {
            'host': host_ip,
            'port': port,
            'user': os.environ.get('MYSQL_USER') or db_config.get('user') or 'root',
            'password': os.environ.get('MYSQL_PASSWORD') or db_config.get('password') or '',
            'database': os.environ.get('MYSQL_DATABASE') or db_config.get('database') or 'hentai',
    ...
    def setup_scraper(self):
        ...
        # 字幕过滤（见 production_config.yml）
        # - no_subtitle_markers: 无字幕条目的特殊标识 → 直接跳过
        # - skip_keywords: 後補等质量问题 → 直接跳过
        # - require_chinese_subtitle: 仅保留有中文字幕证据的条目
        self.NO_SUBTITLE_MARKERS = tuple(crawl_config.get('no_subtitle_markers', [
            '無字幕', '无字幕', '無中字', '无中字',
            '無字', '无字', '生肉', 'RAW',
            '[無字]', '[无字]', '[無字幕]', '[无字幕]', '[無中字]', '[无中字]',
        ]))
        ...
        self.SUBTITLE_POSITIVE_MARKERS = tuple(crawl_config.get('subtitle_positive_markers', [
            '[中文字幕]', '中文字幕', '中文配音', '中字', '简中', '繁中',
            'Chinese Sub', 'CHS', 'CHT',
        ]))
        self.REQUIRE_CHINESE_SUBTITLE = bool(
            crawl_config.get('require_chinese_subtitle', True)
        )
    ...
    def _norm_text(self, *parts: str) -> str:
        return ' '.join((p or '').strip() for p in parts if p and str(p).strip())

    def _contains_any(self, text: str, keywords: tuple[str, ...]) -> str | None:
    ...
    def skip_reason(
        self,
        title: str,
        extra_texts: list[str] | tuple[str, ...] | None = None,
        *,
        stage: str = 'detail',
    ) -> str | None:
        """返回跳过原因；None 表示保留。

        依据 hanime1.me 实际详情页标题抽样（shareBtn-title）:
        - 有中文字幕的里番：标题带方括号标识 ``[中文字幕]``
          例: ``OVA 田舎にはこれくらいしか娯楽がない ＃2 [中文字幕]``
        - 无中文字幕的 UGC/MMD 等：标题**没有** ``[中文字幕]``
          例: ``[RD] Himeko Nova``、``[Tocher] Phrolova x Camellya``
        - 列表页中文 UI 标题通常**不带**该括号（如「鄉下幾乎沒有娛樂活動 2」），
          因此 ``stage='list'`` 时不因缺少 [中文字幕] 而拦截，等详情 raw_title 再判。
        - 站内几乎不用「無字幕」作标题；无字片主要靠**缺少 [中文字幕]** 区分。
        - 「後補/补字」仍作质量屏蔽。
        """
        extras = [str(x) for x in (extra_texts or []) if x]
        blob = self._norm_text(title, *extras)

        # 1) 显式无字幕/生肉标识（若出现）
        hit = self._contains_any(blob, self.NO_SUBTITLE_MARKERS)
        if hit:
            return f'无字幕标识「{hit}」'

        # 2) 後補等质量问题
        hit = self._contains_any(blob, self.SKIP_KEYWORDS)
        if hit:
            return f'屏蔽词「{hit}」'

        if not self.REQUIRE_CHINESE_SUBTITLE:
            return None

        # 3) 中文字幕证据：详情页以 [中文字幕] 为准
        if '[中文字幕]' in blob or re.search(r'\[\s*中文字幕\s*\]', blob):
            return None
        # 标签里的官方属性
        if self._contains_any(' '.join(extras), ('中文字幕', '中文配音')):
            return None
        # 其它正面写法（标题内嵌，非无中字）
        if not re.search(r'[無无]\s*中字|[無无]\s*字幕', blob):
            for kw in ('中文字幕', '中文配音', 'Chinese Sub', 'CHS'):
                if kw.lower() in blob.lower():
                    return None
            # 「中字」需排除「無中字/无中字」
            if re.search(r'(?<![無无])中字', blob):
                return None

        # 列表阶段：中文展示标题常无括号，交给详情页 raw_title 再判
        if stage == 'list':
            return None

        return '详情标题无 [中文字幕] 标识（hanime1 有中字片在 shareBtn-title 上带此标记）'
    ...
    def should_skip(
        ...
        *,
        ...
        """兼容旧调用：是否应跳过。"""
        return self.skip_reason(title, extra_texts=extra_texts, stage=stage) is not None
    ...
    def scrape_hanime_info(self, video_id):
        ...
        try:
            ...
            # 获取标题（保留 [中文字幕] 等括号，过滤后再 strip）
            title_text = tree.xpath('//*[@id="shareBtn-title"]/text()')
            if title_text:
                raw_title = title_text[0].strip()
                title = self.strip_brackets(raw_title)
                info.update({
                    'title': title,
                    'raw_title': raw_title,
                    'originaltitle': title,
