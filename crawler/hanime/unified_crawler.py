...
class UnifiedCrawler:
    ...
    def get_default_config(self):
        ...
        return {
            'crawl': {
                'skip_keywords': [
                    '中字後補', '中文後補', '简中补字', '中文字幕後補', '後補字幕', '後補', '补字',
                ],
    ...
    def setup_scraper(self):
        ...
        # skip_keywords 在去括号之前匹配；空白项忽略
        raw_skip = crawl_config.get('skip_keywords', [
            '中字後補', '中文後補', '简中补字', '中文字幕後補', '後補字幕', '後補', '补字',
        ])
        self.SKIP_KEYWORDS = tuple(
            str(kw).strip() for kw in (raw_skip or []) if str(kw).strip()
        )
    ...
    def matched_skip_keyword(self, *texts: str) -> str | None:
        """在任意文本中查找 skip_keywords；返回命中的关键词，未命中返回 None。

        必须使用「去括号之前」的原始标题（含 [中文後補] 等），
        否则 strip_brackets 会把关键词一起删掉导致永远匹配失败。
        """
    ...
    def should_skip(self, title: str, *extra_texts: str) -> bool:
        """检查标题/附加文本是否包含 skip_keywords 屏蔽词。"""
        return self.matched_skip_keyword(title, *extra_texts) is not None
                ...
                try:
                    ...
                    for idx, match in enumerate(matches, 1):
                        ...
                        try:
                            ...
                            hit = self.matched_skip_keyword(raw_title)
                            if hit:
                                self.logger.info(f"跳过(屏蔽词「{hit}」): {raw_title}")
                                continue
    ...
    def scrape_hanime_info(self, video_id):
        ...
        try:
            ...
            # 获取标题：先保留 raw_title（含 [中文後補]/[中文字幕]），过滤后再 strip
