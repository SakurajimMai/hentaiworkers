...
class ProductionCrawler:
    ...
    def initialize(self):
        ...
        try:
            ...
            self.logger.info(f"skip_keywords: {list(self.crawler.SKIP_KEYWORDS)}")
    ...
    def crawl_month(self, year, month, max_videos, continue_on_error, skip_existing):
        ...
        try:
            ...
            for match in matches:
                ...
                # 列表标题可能为空：空标题不在此拦截，交给详情页用 raw_title 再判
                if title_text:
                    hit = self.crawler.matched_skip_keyword(title_text)
                    if hit:
                        self.logger.info(f"⛔ 早期过滤(屏蔽词「{hit}」): {title_text}")
    ...
    def process_video(self, video_url, cover_map, year, month, skip_existing):
        ...
        try:
            ...
            # 必须用去括号前的 raw_title 做 skip 匹配
            # scrape_hanime_info 里 title 已 strip_brackets，关键词如 [中文後補] 会丢失
            raw_title = (
                video_info.get('raw_title')
                or video_info.get('title')
                or f'video_{video_id}'
            )
            title = video_info.get('title') or self.crawler.strip_brackets(raw_title)
            cn_title = video_info.get('tagline') or ''
            self.logger.info(f"视频标题(raw): {raw_title}")
            if cn_title:
                self.logger.info(f"中文标题: {cn_title}")

            hit = self.crawler.matched_skip_keyword(raw_title, cn_title, title)
            if hit:
                self.logger.info(f"跳过视频（屏蔽词「{hit}」）: {raw_title}")
                self.stats['skipped_videos'] += 1
                return True
