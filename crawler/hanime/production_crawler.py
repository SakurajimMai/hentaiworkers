...
class ProductionCrawler:
    ...
    def crawl_month(self, year, month, max_videos, continue_on_error, skip_existing):
        ...
        try:
            ...
            for match in matches:
                ...
                # 早期字幕/屏蔽过滤（无字幕标识、後補、无中文字幕证据）
                if title_text:
                    reason = self.crawler.skip_reason(title_text, stage='list')
                    if reason:
                        self.logger.info(f"⛔ 早期过滤({reason}): {title_text}")
    ...
    def process_video(self, video_url, cover_map, year, month, skip_existing):
        ...
        try:
            ...
            raw_title = video_info.get('raw_title') or title
            cn_title = video_info.get('tagline', '') or ''
            tags = video_info.get('tag', []) or video_info.get('genre', []) or []
            self.logger.info(f"视频标题: {raw_title}")
            if cn_title:
                self.logger.info(f"中文标题: {cn_title}")

            # 过滤：无字幕标识 / 後補 / 无中文字幕证据
            skip_reason = self.crawler.skip_reason(
                raw_title,
                extra_texts=[cn_title, title, *list(tags)],
                stage='detail',
            )
            if skip_reason:
                self.logger.info(f"跳过视频（{skip_reason}）: {raw_title}")
                self.stats['skipped_videos'] += 1
                return True
